<div class="tab-pane fade @if($activeHomeSection === 'testimonials') show active @endif" id="testimonials-pane" role="tabpanel" aria-labelledby="testimonials-tab" tabindex="0">
    <div class="mb-3">
        <div class="form-check form-switch">
            <input class="form-check-input" type="checkbox" role="switch"
                   name="home_testimonials_is_enabled" value="1" id="home_testimonials_is_enabled"
                   {{ old('home_testimonials_is_enabled', array_key_exists('is_enabled', $testimonialsSection ?? []) ? !empty($testimonialsSection['is_enabled']) : true) ? 'checked' : '' }}>
            <label class="form-check-label fw-semibold" for="home_testimonials_is_enabled">Show Testimonials Section on Home Page</label>
        </div>
        <div class="form-text">
            Left panel image and headings are set here. Review quotes and client details come from active items in <a href="{{ route('admin.testimonials.index') }}">Testimonials</a>.
        </div>
    </div>
    ...
    <div class="row g-3">
        ...
        <div class="col-md-8">
            <label class="form-label">Left panel image</label>
