                ...
                <form action="{{ route('admin.profile.update') }}" method="POST" enctype="multipart/form-data" id="profile-form">
                    ...
                    <div class="mt-4 d-flex flex-wrap gap-2 align-items-center">
                        ...
                        <span class="text-muted small">
                            @if($emailChangePending)
                                Saves name, photo, and password. Email updates only after you enter the code below.
                            @else
                                Name, photo, and email save here; password only if you filled that section.
                            @endif
                        </span>
                    </div>
                </form>
                ...
                @if($emailChangePending && $emailChangeOtpExpiresAt && ! $emailChangeLocked)
                    <div class="account-otp-card" @if(! $canResendEmailOtp && $emailChangeOtpResendAvailableAt) data-resend-until="{{ $emailChangeOtpResendAvailableAt->toIso8601String() }}" @endif>
                        ...
                        <div class="otp-actions mt-2 pt-3">
                            ...
                            <form action="{{ route('admin.profile.cancel-email-change') }}" method="POST" class="d-inline m-0" onsubmit="return confirm('Cancel this email change? Your login address will stay as it is now.');">
                                @csrf
