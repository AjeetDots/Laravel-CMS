...
<head>
    ...
    @php
        $themeWine = trim((string) ($settings->get('theme_wine') ?? ''));
        $themeWineDark = trim((string) ($settings->get('theme_wine_dark') ?? ''));
        $themeGold = trim((string) ($settings->get('theme_gold') ?? ''));
    @endphp
    @if($themeWine !== '' || $themeWineDark !== '' || $themeGold !== '')
    <style id="site-theme-css-overrides">
        :root {
            @if($themeWine !== '')
            --wine: {{ $themeWine }};
            --blue: {{ $themeWine }};
            --wine-light: color-mix(in srgb, {{ $themeWine }} 14%, white);
            --blue-light: var(--wine-light);
            @endif
            @if($themeWineDark !== '')
            --wine-dark: {{ $themeWineDark }};
            --blue-dark: {{ $themeWineDark }};
            @endif
            @if($themeGold !== '')
            --gold: {{ $themeGold }};
            --gold-light: color-mix(in srgb, {{ $themeGold }} 18%, white);
            @endif
            @if($themeWine !== '')
            --gradient: linear-gradient(135deg, var(--wine) 0%, color-mix(in srgb, var(--wine) 72%, white) 100%);
            @endif
            @if($themeWine !== '' && $themeGold !== '')
            --violet: color-mix(in srgb, var(--gold) 55%, var(--wine) 45%);
            @endif
        }
    </style>
    @endif
...
<footer class="site-footer">
    <div class="container footer-main">
        <div class="row g-5 g-xl-5 align-items-start">
            ...
            {{-- Footer columns: managed under Admin → Appearance → Menus (top-level + "Footer column") --}}
