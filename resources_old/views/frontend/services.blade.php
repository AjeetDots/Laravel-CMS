...
@section('title', trim((string) ($servicesPage['page_title'] ?? '')) !== '' ? $servicesPage['page_title'] : 'Services')
...
@php $stripeEven = (($services->firstItem() ?? 1) + $loop->index) % 2 === 0; @endphp
<section class="svc-split {{ $stripeEven ? 'svc-split--flip' : '' }}">
    <div class="container">
        <div class="row g-4 g-lg-5 align-items-center">
            <div class="col-lg-6 {{ $stripeEven ? 'order-lg-2' : '' }}">
            ...
            <div class="col-lg-6 {{ $stripeEven ? 'order-lg-1' : '' }}">
...
<div class="container text-center py-5 services-empty-state">
    ...
    @if(!empty(trim($servicesPage['empty_btn_text'] ?? '')))
        <a href="{{ \App\Support\CmsOutboundHref::resolve(!empty(trim($servicesPage['empty_btn_href'] ?? '')) ? $servicesPage['empty_btn_href'] : null, 'contact') }}" class="btn-outline-site">{{ $servicesPage['empty_btn_text'] }}</a>
    @endif
</div>
@endforelse

@if($services instanceof \Illuminate\Contracts\Pagination\Paginator && $services->hasPages())
<div class="container py-3">
    <div class="d-flex justify-content-center">{{ $services->withQueryString()->links() }}</div>
</div>
@endif
...
<div class="cta-strip">
    <div class="container text-center">
        ...
        @endif
        @if(!empty(trim($servicesPage['bottom_btn_text'] ?? '')))
        <a href="{{ \App\Support\CmsOutboundHref::resolve(!empty(trim($servicesPage['bottom_btn_href'] ?? '')) ? $servicesPage['bottom_btn_href'] : null, 'contact') }}" class="btn-white">{{ $servicesPage['bottom_btn_text'] }} <i class="fas fa-arrow-right" style="font-size:.75rem;" aria-hidden="true"></i></a>
        @endif
    </div>
</div>
