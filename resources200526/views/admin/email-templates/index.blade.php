    ...
    @foreach($audienceLabels as $key => $label)
        <li class="nav-item me-2">
            <a class="nav-link {{ $audience === $key ? 'active' : '' }}"
               href="{{ route('admin.email-templates.index', array_filter(['audience' => $key, 'q' => request('q'), 'status' => request('status')])) }}">
                {{ $label }}
            </a>
        </li>
    @endforeach
...
@include('admin.partials.listing-toolbar', [
    'showStatus' => true,
    'hiddenFields' => ['audience' => $audience],
])

<div class="card">
    <div class="card-header">{{ $audienceLabels[$audience] ?? 'Templates' }} ({{ $templates->count() }} total)</div>
            ...
            <table class="table mb-0" data-admin-dt>
                <thead>
                    <tr>
                        ...
                        <th width="70" class="text-center" data-dt-orderable="false">Edit</th>
                    </tr>
                </thead>
                <tbody>
